...
設定は目的別の4カテゴリに整理されています：

- **Display（表示）** — 外観・ターミナル表示・画面
- **Behavior（操作）** — キーオーバーレイ・入力・スクロール送信・ペイン操作
- **Connection & Transfer（接続と転送）** — SSH接続・画像転送
- **About（このアプリについて）** — アプリ情報・ライセンス

**設定の検索**（画面上部）は全カテゴリの全項目を英語・日本語の両方で横断検索し、項目名から直接ジャンプできます。

レイアウトはレスポンシブ対応です。スマホではカテゴリをタップすると詳細が全画面表示になり、タブレットや横持ちの広い画面ではカテゴリ一覧と詳細が左右2ペイン（マスター・ディテール）表示になります。
...
ターミナルオプションメニューから排他的な 3 モードを選択できます:

- **Normal** — キー入力とライブ表示（デフォルト）。
- **Scroll Send** — ドラッグでアプリ（ペイン内で動作中）へスクロール（ホイール / PgUp / PgDn）を送信。ローカルスクロールとテキスト選択は無効。
- **Select** — 履歴閲覧とテキスト選択。選択中はターミナル更新がバッファリングされるのでコンテンツがジャンプしません。選択テキストはシステムクリップボードにコピー。
