...
class _ConnectionFormScreenState extends ConsumerState<ConnectionFormScreen> {
  ...
  AppBar _buildAppBar() {
    ...
    return AppBar(
      ...
      leadingWidth: 80,
      title: Text(
        widget.isEditing ? context.l10n.connEditTitle : context.l10n.connAddTitle,
        style: GoogleFonts.spaceGrotesk(
        ...
        TextFormField(
          ...
          validator: (value) {
            if (value != null && value.isNotEmpty && !value.startsWith('/')) {
              return isHerdr
                  ? context.l10n.connAbsolutePathRequired('/usr/local/bin/herdr')
    ...
    return Column(
      ...
      children: [
        DropdownButtonFormField<String>(
          ...
          hint: Text(
            keysState.keys.isEmpty ? context.l10n.connNoKeysAvailable : context.l10n.connSelectKey,
            style: GoogleFonts.spaceGrotesk(color: mutedColor),
          ),
        ),
        if (_authMethod == 'key' && keysState.keys.isEmpty) ...[const SizedBox(height: 8), Text(context.l10n.connNoKeysFoundAddInKeys, style: GoogleFonts.spaceGrotesk(fontSize: 12, color: colorScheme.error))],
        if (_authMethod == 'key' &&
            _selectedKeyId != null &&
    ...
    if (mounted) {
      ...
      if (errorMessage != null) {
        ...
        final message = tmuxInstalled
            ...
            : l10n.connTestSuccessWarning(
                tmuxWarning ?? l10n.connTmuxNotFound,
              );
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(message),
