...
void main() {
  ...
  group('SshProxyOptionsResolver MR-8 contract', () {
    ...
    test(
      'missing hop password throws fail-fast with hop coordinates',
      () async {
        ...
        await expectLater(
          ...
          throwsA(
            isA<SshProxyConnectionError>()
                ...
                .having((e) => e.message, 'message', contains('jump1')),
          ),
        );
      },
    );
    ...
    test('key hop without keyId throws', () async {
      const proxy = ProxyConfig(
        hops: [ProxyHop(host: 'jump1', username: 'u1', authMethod: 'key')],
      );
  ...
  group('SshProxyOptionsResolver resolution', () {
    test(
      'resolves password and key hops in chain order (M3 forward)',
      () async {
    ...
    test(
      'forwardPort only (blank forwardHost) falls back to target (M6)',
      () async {
        SecureStorageService.setTestValues({'proxy_password_conn1_0': 'pw0'});
        const proxy = ProxyConfig(
          hops: [ProxyHop(host: 'jump1', username: 'u1')],
    ...
    test(
      'explicit forwardHost with unset forwardPort uses target port',
      () async {
        SecureStorageService.setTestValues({'proxy_password_conn1_0': 'pw0'});
        const proxy = ProxyConfig(
          hops: [ProxyHop(host: 'jump1', username: 'u1')],
