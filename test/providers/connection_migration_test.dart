...
void main() {
  group('ConnectionProvider', () {
    ...
    group('proxy / keepalive schema (Issue #56)', () {
      test(
        'proxy & keepalive record stays v2 and migration is not triggered',
        () async {
          final storage = SecureStorageService();
          final v2Json = jsonEncode([
            {
      ...
      test(
        'downgrade round trip drops proxy and keepalive (documented loss)',
        () {
          const hop = ProxyHop(host: 'bastion.example.com', username: 'jump');
          final connection = Connection(
            id: 'c1',
            name: 'Server',
            host: 'h',
