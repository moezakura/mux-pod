...
void main() {
  ...
  group('AnsiTextView scrollSend display branch (C5・D5)', () {
    ...
    testWidgets(
      'TERM-SCROLL-011 drag down emits negative ticks',
      (tester) async {
        final ticks = <int>[];
        await tester.pumpWidget(
          _buildSubject(
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(ticks, isNotEmpty,
            reason: 'ドラッグはスクロールではなくティック送信へ変換される');
      },
    );

    testWidgets(
      'select mode keeps SelectionArea (4 経路分離・D12)',
      (tester) async {
        await tester.pumpWidget(
    ...
    testWidgets(
      'TERM-SCROLL-014 two-finger pan does not trigger pane swipe in '
      'scrollSend (M5)',
      (tester) async {
        final ticks = <int>[];
    ...
    testWidgets(
      'normal mode drag does not emit ticks (scrollSend 分岐のみ)',
      (tester) async {
        final ticks = <int>[];
        await tester.pumpWidget(
        ...
        expect(ticks, isEmpty,
            reason: 'normal モードは既存のリストスクロール（ティック送信なし）');
      },
    );
  });
}
