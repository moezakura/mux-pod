...
void main() {
  group('scrollSend P0: モード状態機械・単一選択・原子性（D9/C1）', () {
    ...
    testWidgets(
      'TERM-SCROLL-018 原子性契約: scrollSend 入口で source==none・バッファ空',
      (tester) async {
        await TerminalTestScaffold.pumpTerminalScreen(
          tester,
          settings: _kKeySendSettings,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          client.sendKeysCommands.any((c) => c.contains('\x1b[5~')),
          isTrue,
    ...
    testWidgets(
      'TERM-SCROLL-020 送信方式ゲート（D11）: wheel 有効時 SGR・key へフォールバック',
      (tester) async {
        final client = await TerminalTestScaffold.pumpTerminalScreen(
          tester,
        ...
        await tester.pump();
        expect(
          client.sendKeysCommands.any((c) => c.contains('\x1b[<64;1;1M')),
          isTrue,
  ...
  group('scrollSend P1: 合流送信・キー入力・方向反転（D6/H3/L0-a #4/#6）', () {
    testWidgets(
      'TERM-SCROLL-021 合流送信: 最大 8 ティックを 1 コマンドに連結し超過分は保持',
      (tester) async {
        final client = await TerminalTestScaffold.pumpTerminalScreen(
          tester,
          settings: _kKeySendSettings,
        ...
        expect(client.sendKeysCommands, isEmpty,
            reason: 'flush 前に送信されない');

        // 1 回目の flush: 最大 8 ティックを 1 コマンドに連結、超過分（2）は保持。
        await tester.pump(const Duration(milliseconds: 100));
        await tester.pump();
        expect(
          client.sendKeysCommands.any((c) => c.contains('\x1b[5~' * 8)),
          isTrue,
          reason: '8 ティック分が 1 コマンドに連結される（D6）',
        );
        expect(
          client.sendKeysCommands.any((c) => c.contains('\x1b[5~' * 9)),
          isFalse,
        ...
        await tester.pump();
        expect(
          client.sendKeysCommands.any((c) => c.contains('\x1b[5~' * 2)),
          isTrue,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pump();
        expect(
          client.sendKeysCommands.any((c) => c.contains('\x1b[6~')),
          isTrue,
    ...
    testWidgets(
      'TERM-SCROLL-023 文字キーは sendText 経由・オーバーレイなし・キューなし',
      (tester) async {
        final client = await TerminalTestScaffold.pumpTerminalScreen(
          tester,
          settings: _kKeySendSettings,
        ...
        await tester.pump();
        expect(client.sendKeysCommands, isEmpty,
            reason: '未接続時は即ドロップしキューに積まない');
      },
    );

    testWidgets(
      'TERM-SCROLL-024 方向反転設定 ON でドラッグ方向が反転する（L0-a #4）',
      (tester) async {
        final client = await TerminalTestScaffold.pumpTerminalScreen(
          tester,
  ...
  group('scrollSend P2: 異常系（R6/M4/H4②）', () {
    testWidgets(
      'TERM-SCROLL-025 切断時は合流ティックを破棄し送信しない（R6）',
      (tester) async {
        final client = await TerminalTestScaffold.pumpTerminalScreen(
          tester,
          settings: _kKeySendSettings,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pump();
        client.sendKeysCommands.clear();
        client.execOutputs['capture-pane'] = 'copy body\n11,12,100,40\ncopy-mode';
        ...
        final dynamic state = _state(tester);
        final events =
            (state.herdrSwitchEventsForTesting() as List).cast<String>();
        expect(
    ...
    testWidgets(
      'TERM-SCROLL-027 モード切替で合流タイマー cancel・保留ティック破棄（M4）',
      (tester) async {
        final client = await TerminalTestScaffold.pumpTerminalScreen(
          tester,
          settings: _kKeySendSettings,
